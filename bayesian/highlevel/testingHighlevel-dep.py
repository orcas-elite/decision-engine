...
probMatrix=pd.read_csv('probhighlevel-dep.csv')
...
# count of trials to run
trials = 50
# count of experiments to run in a trial
nExperiments = 100
...
for trialno in range(trials):
    ...
    for i in range(nExperiments):
        ...
        # transform string representation of list to actual list
        selDep = ast.literal_eval(selDep)
        ...
        # DataFrame seems to be having some issues when it comes to using .loc with list values, i.e. archInfoDf['dependencies']==selDep

        patMatch = archInfoDf.loc[(archInfoDf['patterns']==selPat)]
        posCases = []
        for index,row in patMatch.iterrows():
            if selDep == row[1]:
                posCases.append(row)
        ...
        # posFaults=faultInjectionDF.loc[(faultInjectionDF['faults'])]
        # get matching operations for patterns/dependencies
        # choose sample from matching fault injections

        # randomrow: [OPERATION, DEPENDENCIES, PATTERNS]
        randomrow = random.choice(posCases)
        ...
        if injectcomp in exComps:
            ...
            comPat='false' if randomrow[2]==0 else 'true'
        ...
        # TODO preload these to improve performance?
