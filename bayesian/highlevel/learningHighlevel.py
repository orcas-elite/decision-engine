...
df=pd.read_csv('postProcessHighLevel.csv')
...
for pat in probMatrix['patterns']:
    ...
    for faultIn in probMatrix['errorInjected']:
        ...
        for dep in probMatrix['dependencies']:
            ...
            # Check if combination of pattern, faultType, dependencies exists in experiment data (postProcessHighLevel.csv)
            ...
            if(len(total) > 0):
                ...
                #if(probMatrix.loc[((probMatrix['patterns']==pat) & (probMatrix['errorInjected']==faultIn)& (df['dependencies']==dep))].iloc[0]['probabilities']==selP):
...
probMatrix.to_csv('probhighlevel.csv')
